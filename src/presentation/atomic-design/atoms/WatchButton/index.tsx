...
const WatchButton = ({ label, onClick, width, className, hasIcon = false }: WatchButtonProps) => {
    return (
        <button onClick={onClick} className={clsx(`bg-[#ff0068] sm:text-base text-xs px-4 py-2 flex justify-center items-center gap-3 rounded-md`, width || 'w-full', className)}>{label}
          {hasIcon && (
